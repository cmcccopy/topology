...
function fem(nelx::Int,nely::Int,x::Matrix{Float64},penal::Number)
    ...
    F[2*(nely+1)*(nelx+1)-(nely)]=-1#向下的力
    fixdofs=1:2*(nely+1)#固定的自由度标号
...
nelx=50;nely=30; #x方向，y方向单元个数
penal=3.         #x的惩罚因子
rmin=2           #网格灵敏度过滤半径
volfrac=0.5      #体积分数
x,c,l=top(nelx,nely,volfrac,penal,rmin)
picture_c=plot(1:l,c,title = " cantilever beam", label = " flexibility") 
xlabel!("iteration")
picture_x=heatmap(x) #绘制矩阵的热图
savefig(picture_c,"picture_c.png")
savefig(picture_x,"picture_x.png")
